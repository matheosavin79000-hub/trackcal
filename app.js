...
// Identifiants JSONBin par défaut — partagés entre tous vos appareils,
// pour que la synchronisation fonctionne sans configuration manuelle.
const DEFAULT_JSONBIN_KEY = '$2a$10$wtxdYtjUfe3XYqNdq7bKCul8rN5/X/J1V04ideTK6g9Nx7Daf0V86';
const DEFAULT_JSONBIN_ID  = '6a3833ddda38895dfee7e26b';
...
function setupOnboarding(){
  ...
  $('#profile-form').addEventListener('submit',async e=>{
    ...
    const jsonbinKey=fd.get('jsonbinKey')||DEFAULT_JSONBIN_KEY;
    const jsonbinId=fd.get('jsonbinId')||DEFAULT_JSONBIN_ID;
...
function setupSyncSettings(){
  ...
  openBtn.addEventListener('click',()=>{
    $('#sync-jsonbin-key').value=DB.profile?.jsonbinKey||DEFAULT_JSONBIN_KEY;
    $('#sync-jsonbin-id').value=DB.profile?.jsonbinId||DEFAULT_JSONBIN_ID;
...
// Determine start state once authenticated:
// always try to fetch the remote profile/data first (default JSONBin credentials),
// so a brand-new device recovers existing data instead of showing onboarding.
async function boot(){
  // Local profile already present (this device has used the app before) → go straight in,
  // a background sync below will still pick up any newer remote entries.
  if(DB.profile){
    DB.profile.jsonbinKey = DEFAULT_JSONBIN_KEY;
    DB.profile.jsonbinId  = DEFAULT_JSONBIN_ID;
    Store.save(DB);
    showAppScreen();
    backgroundSync();
    return;
  ...
  // No local profile (new device) → never show onboarding, just keep trying to fetch
  // the single shared profile from the cloud until it succeeds.
  $('#screen-password').classList.add('hidden');
  $('#onboarding').classList.add('hidden');
  $('#app').classList.add('hidden');
  ...
  const tryFetch=async()=>{
    statusEl.textContent='Récupération de vos données…';
    retryBtn.classList.add('hidden');
    const remote=await Store.pull(DEFAULT_JSONBIN_KEY, DEFAULT_JSONBIN_ID);
    if(remote?.profile){
      DB = remote;
      DB.profile.jsonbinKey = DEFAULT_JSONBIN_KEY;
      DB.profile.jsonbinId  = DEFAULT_JSONBIN_ID;
      ...
      statusEl.textContent='⚠️ Impossible de récupérer le profil (réseau indisponible ou aucune donnée trouvée).';
      retryBtn.classList.remove('hidden');
    }
  };
  retryBtn.addEventListener('click', tryFetch);
  tryFetch();
}
